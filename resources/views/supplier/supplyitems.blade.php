                ...
                @foreach($supplyItems as $item)
                    <tr>
                        ...
                        <td>
                            ...
                            <!-- Delete Button -->
                            <form action="{{ route('supplier.supplyitems.delete', $item->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirmDelete()">
                                @csrf
                                @method('DELETE')
...
<div class="modal fade" id="editSupplyModal" tabindex="-1" aria-labelledby="editSupplyModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editSupplyModalLabel">Edit Supply Item</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <!-- Form without JavaScript, using POST method -->
            <form action="{{ route('supplier.supplyitems.update', $item->id) }}" method="POST"> <!-- Directly submit to update route -->
                @csrf
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" id="edit_name" name="name" class="form-control" value="{{ $item->name }}" required>
                    </div>

                    <div class="form-group">
                        <label for="edit_description">Description</label>
                        <textarea id="edit_description" name="description" class="form-control">{{ $item->description }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="edit_price">Price (per unit)</label>
                        <input type="number" id="edit_price" step="0.01" name="price" class="form-control" value="{{ $item->price }}" required>
                    </div>

                    <div class="form-group">
                        <label for="edit_stock_quantity">Stock Quantity</label>
                        <input type="number" id="edit_stock_quantity" name="stock_quantity" class="form-control" value="{{ $item->stock_quantity }}" required min="1">
                    </div>
                </div>
...
<!-- JavaScript to handle confirmation for edit, delete, and price validation -->
@section('scripts')
...
<script>
    $(document).ready(function() {
        ...
        // Populate the edit modal with the data from the clicked row
        $('#editSupplyModal').on('show.bs.modal', function (event) {
            ...
            modal.find('#edit_stock_quantity').val(stock);

            // Set the form action to the correct update route with the item ID
            var form = $('#editSupplyForm');
            form.attr('action', '/supplier/supplyitems/' + id + '/update');
        });
    });
</script>
