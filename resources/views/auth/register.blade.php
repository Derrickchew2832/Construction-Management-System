<x-guest-layout>
    <form method="POST" action="{{ route('register') }}" enctype="multipart/form-data">
        ...
        <div class="mt-4">
            ...
            <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone')" required autocomplete="phone" pattern="[0-9]{10}" title="Please enter a valid 10-digit phone number" />
            @error('phone')
                <x-input-error :messages="$message" class="mt-2" />
            @enderror
        </div>
        ...
        <div class="mt-4">
            ...
            <input id="document" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" type="file" name="document" required accept=".pdf,.doc,.docx" />
            @error('document')
                <x-input-error :messages="$message" class="mt-2" />
            @enderror
        </div>
